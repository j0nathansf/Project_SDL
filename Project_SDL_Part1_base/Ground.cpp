...
void Ground::initAnimals()
{
    for (int i = 0; i < 3; i++)
    {
        this->animals.push_back(new Sheep(rand() % SCREEN_WIDTH, rand() % SCREEN_HEIGHT, this->renderer));
    }

    for (int i = 0; i < 3; i++)
    {
        this->animals.push_back(new Wolf(rand() % SCREEN_WIDTH, rand() % SCREEN_HEIGHT, this->renderer));
    }
}
